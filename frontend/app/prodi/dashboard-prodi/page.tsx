...
const DashboardProdiPage = () => {
  ...
  const onInputLED = (id: string) => alert(`Input LED ${id} coming soon...`);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#f4f6f8] p-5">
        <div className="bg-white max-w-md w-full rounded-xl shadow-sm border border-orange-100 p-8 text-center text-[#132040]">
          ...
          <p className="text-sm text-gray-600 mb-6">Akun Anda tidak terhubung dengan Program Studi manapun. Silakan hubungi admin.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 px-10 bg-[#f4f6f8] min-h-screen">
      ...
      {!profilProdi.is_active_accreditation && (
        <div className="mb-5 bg-gray-100 border border-gray-300 rounded-xl px-5 py-3 flex items-center gap-3 text-sm text-gray-600">
          ...
          <div>
            <p className="font-bold text-gray-700">Prodi tidak aktif dalam siklus akreditasi tahun ini</p>
            <p>Hubungi admin untuk mengaktifkan prodi pada siklus akreditasi ini.</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-base font-bold text-[#132040]">Profil Program Studi</h2>
        ...
        <div className="flex flex-wrap gap-6 text-sm">
          ...
          <div>
            ...
            <p className="font-semibold text-gray-800">
              ...
              <span className="text-gray-400 font-normal ml-1">({profilProdi.last_accreditation_year})</span>
            </p>
          </div>
          <div>
            <p className="text-xs text-gray-400 mb-0.5">Status Siklus Ini</p>
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${profilProdi.is_active_accreditation ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-500"
              }`}>
      ...
      <div className="flex gap-6 flex-wrap">
        <div className="flex-1 min-w-[320px] flex flex-col gap-4">
            ...
            <div className="space-y-3 mb-5">
              {[
                ...
                <div key={label} className="flex items-center gap-3">
                  <span className="text-sm text-gray-600 font-medium w-36 shrink-0">{label}</span>
              ...
              <table className="w-full text-sm relative">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-[#00509d] text-white">
                    ...
                    {editable && (
                      <th className="px-4 py-3 font-semibold">Isi / Input Dokumen</th>
                    )}
                  </tr>
                </thead>
                <tbody>
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            {row.has_lkps && (
                              <button
                                ...
                                className={`text-xs rounded-md px-2.5 py-1 transition-colors font-medium border ${row.lkps_available
                            ...
                            {row.has_led && (
                              <button
                                ...
                                className={`text-xs rounded-md px-2.5 py-1 transition-colors font-medium border ${row.led_available
                                  ? "border-[#00509d] text-[#00509d] hover:bg-blue-50"
                                  : "border-orange-400 text-orange-600 hover:bg-orange-50"
                                  }`}
                              >
          ...
          {!editable && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-bold text-[#132040]">Komentar Pimpinan</p>
              </div>
              <textarea
          ...
          <div className="bg-blue-50 border-l-4 border-[#00509d] rounded-r-xl p-4">
            <p className="text-sm font-bold text-[#132040] mb-2">💡 Pesan Rekomendasi</p>
            <ul className="list-disc list-inside space-y-1.5">
              {pesanRekomendasi.map((msg: string, i: number) => (
                <li key={i} className="text-xs text-gray-700 leading-relaxed">{msg}</li>
              ))}
            </ul>
          </div>
        </div>
          ...
          <div className="bg-white border border-red-200 rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-2 text-xs text-gray-700">
              <div className="flex items-start gap-2 bg-red-50 rounded-lg px-3 py-2">
                <span>⏰</span>
                <p>Deadline: <strong className="text-red-600">{deadline}</strong></p>
              </div>
              <div className="flex items-start gap-2 bg-red-50 rounded-lg px-3 py-2">
                <span>⌛</span>
                <p>Sisa <strong className="text-red-600">{sisaHari}</strong> hari lagi</p>
              </div>
